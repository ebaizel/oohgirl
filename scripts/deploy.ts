...
async function main() {
  ...
  const rentals = await Rentals.deploy();

  console.log("Rentals deployed to:", rentals.address);
  ...
  const ooohGirl = await OoohGirl.deploy(rentals.address, catalog.address);

  await ooohGirl.deployed();

  console.log("OoohGirl deployed to:", ooohGirl.address);
}
